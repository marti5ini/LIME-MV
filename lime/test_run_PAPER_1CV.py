...
def prepare_datasets(namelist=['iris', 'titanic', 'german_credit', 'adult', 'diabetes', 'fico', 'compas-scores-two-years'], cross_validation=False, completely_random = True, one_time =True):
    start = time.time()
    ...
    for dfname in dfnames_list:
        ...
        if cross_validation == False:
            if one_time == True:
                print("Doing ONE TIME validation on complete data... please wait")
                hgb_modelMV = hgb_cross_validation(scaled_X_train, y_train, to_encode=to_encode, obj=obj )
            else:
                print("Skipping cross validation completely.")
        ...
        for col_perc in col_percentages:
            if completely_random==True:
                ...
                for mv_perc in mv_percentages:
                    ...
                    predict_fn = lambda x: trained_hgbMV.predict_proba(x)
                    currname = 'MCAR_'+str(int(col_perc*100))+'_'+str(int(mv_perc*100))+'_LimeMV_ContDist_RUNIF_TreeSigns'
                    cycle_rows_paper(number_of_rows, X_testMV, explainer, predict_fn, colidx, percentages, results_path, start, class_encoding, target_name, 
                                    trained_hgbMV, name=currname, lime='mv', regressor='tree')
                    ...
                    currname = 'MCAR_'+str(int(col_perc*100))+'_'+str(int(mv_perc*100))+'_LimeBASE_ImpOutside_MICE'
        ...
        for col_perc in col_percentages:
            if completely_random==False:
                ...
                for mv_perc in mv_percentages:
                    ...
                    predict_fn = lambda x: trained_hgbMV.predict_proba(x)
                    currname = 'MAR_'+str(int(col_perc*100))+'_'+str(int(mv_perc*100))+'_LimeMV_ContDist_MICE_TreeSigns'
                    cycle_rows_paper(number_of_rows, X_testMV, explainer, predict_fn, colidx, percentages, results_path, start, class_encoding, target_name, 
                                    trained_hgbMV, name=currname, lime='mv', regressor='tree')

                    #OUR METHOD WITH RUNIF GENERATION (7c)
                    print('Testing LimeMV. Seconds passed:', round(time.time()-start, 4), '\n')
                    explainer = limeMV.LimeTabularExplainerMV(X_trainMV, #select correct train.
                                            class_names=class_names,
                                                feature_names=col_names, categorical_features=to_encode,
                                            ...
                                            tree_explainer= 'signs', 
                                            distance_type= 'continuous',
                                            custom_sample= 'uniform', 
                                            sample_removal= 'auto',
                                            to_round=to_encode)

                    predict_fn = lambda x: trained_hgbMV.predict_proba(x)
                    currname = 'MAR_'+str(int(col_perc*100))+'_'+str(int(mv_perc*100))+'_LimeMV_ContDist_RUNIF_TreeSigns'
                    cycle_rows_paper(number_of_rows, X_testMV, explainer, predict_fn, colidx, percentages, results_path, start, class_encoding, target_name, 
                                    trained_hgbMV, name=currname, lime='mv', regressor='tree')
                    ...
                    #ALLENA MICE MODEL!!!!! trained_hgbMICE
                    if cross_validation:
                        print("Doing cross validaiton on complete data... please wait")
                        hgb_modelMICE = hgb_cross_validation(mice_train, y_train, to_encode=to_encode, obj=obj )
                    else:
                    ...
                    predict_fn = lambda x: trained_hgbMICE.predict_proba(x)
                    currname = 'MAR_'+str(int(col_perc*100))+'_'+str(int(mv_perc*100))+'_LimeBASE_ImpOutside_MICE'
                    cycle_rows_paper(number_of_rows, test_dataset_mice, explainer, predict_fn, colidx, percentages, results_path, start, class_encoding, target_name, 
                                    trained_hgbMV, name=currname, lime='base')


if __name__== '__main__':
    prepare_datasets(namelist=['titanic'], cross_validation=False, one_time = True)
