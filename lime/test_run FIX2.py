...
def select_mv_strategy(X_train, y_train, double=True, all=False, max=None, standard=None):
    configurations = []
...
def parameter_grid():
    grid = {(False, False): [], #BASELINES
           ...
           (False, True): [],
           (True, True): []}
    #build a grid... what are the parameters? Set as BASE ones.
    base_parameters = {
        ...
        'custom_sample' : None,
        'sample_removal' : None, #[0]*len(scaled_X_train.transpose()), #number of columns
        'name' : '',
        'pos' : (False, False)
    }
############################################################################################################################
    #OPTIONS
    custom_samples = ['mean', 'knn', 'uniform', 'normal', 'mice'] #
    #custom_samples = ['mice']
    surrogate_distances = [('binary', 'binary'), ('continuous', 'binary'), ('continuous', 'continuous')]
    tree = [False, True, 'signs']
    removals = [None, 'auto']
############################################################################################################################  
     #%/%/%/%/%/%%/ 1 - MV IN THE TEST SET ONLY %/%/%/%/%/%%/
    pos = (False, True)
    #parameters['test_rows'] = X_test_rowsMV
    for cs in custom_samples:
        for sd in surrogate_distances:
            new_parameters = base_parameters.copy()
            new_parameters['custom_sample'] = cs
            new_parameters['surrogate_data'] = sd[0]
            new_parameters['distance_type'] = sd[1]
            new_parameters['pos'] = pos
            new_parameters['name'] = combination_name(new_parameters)
            #print('Appending', parameters['name'], '\n')
            grid[pos].append(new_parameters)
############################################################################################################################            
    #%/%/%/%/%/%%/ 2 - MV IN THE TRAIN SET ONLY %/%/%/%/%/%%/
    #Reset
    #parameters = base_parameters.copy()
    pos = (True, False)
    #parameters['train'] = X_trainMV
    
    ############## 2 - Scenario A: Do not use sample removal
    for sd in surrogate_distances:
        for tr in tree[1:]:
            new_parameters = base_parameters.copy()
            new_parameters['pos'] = pos
            #Here we use tree explainer even it is not necessary. Can cut if needed.
            if tr != False:
                new_parameters['model_regressor'] = 'tree'
                new_parameters['tree_explainer'] = tr
            else:
                new_parameters['model_regressor'] = None
                new_parameters['tree_explainer'] = False
            new_parameters['surrogate_data'] = sd[0]
            new_parameters['distance_type'] = sd[1]
            new_parameters['name'] = combination_name(new_parameters)
            #print('Appending', parameters['name'], '\n')
...
############################################################################################################################            
    ...
    #parameters['pos'] = pos
    #parameters['train'] = X_trainMV
    ############## 2 - Scenario B: Use sample removal
    ...
    for sd in surrogate_distances:
        for tr in tree[1:]:
            ...
            new_parameters['pos'] = pos
            new_parameters['model_regressor'] = 'tree'
            new_parameters['sample_removal'] = 'auto'
            new_parameters['tree_explainer'] = tr
            new_parameters['surrogate_data'] = sd[0]
            new_parameters['distance_type'] = sd[1]
            new_parameters['name'] = combination_name(new_parameters)
        ...
        for sd in surrogate_distances:
            for re in removals:
                for tr in tree:
                    ...
                    new_parameters['custom_sample'] = cs
                    #Treat MVs in TRAIN
                    new_parameters['sample_removal'] = re
                    #end
                    new_parameters['name'] = combination_name(new_parameters)
                    #print('Appending', parameters['name'], '\n')
                    grid[pos].append(new_parameters)
...
############################################################################################################################    
    #%/%/%/%/%/%%/ 0 - BASELINES %/%/%/%/%/%%/ Remember to use LimeBase here
    #QUESTO VA FATTO A MANO DI LA', TANTO è SEMPRE UGUALE
    #DEVO RITRAINARE I MODELLI, IMPUTANDO IN MODO DIVERSO.
    #pos = (False, False)
    #Ground Truth
    pos = (False, False)
    parameters = {'pos': pos}
    
    imp_method = [None, 'mean', 'knn', 'mice']#, 'extra']
    #imp_method = ['mice']
    imp_when = ['before', 'after']
    for method in imp_method:
        if method is not None:
            for wh in imp_when:
                new_parameters = parameters.copy()
                new_parameters['imputation_method'] = method
                new_parameters['imputation_when'] = wh
                new_parameters['name'] = combination_name_base(new_parameters)
                #print('Appending', new_parameters['name'], '\n')
                ##print(parameters)
                #print('BEFORE', grid[pos], 'BEFORE')
                grid[pos].append(new_parameters)
                #print('AFTER', grid[pos], 'AFTER')
        else:
            #True Baseline 
            new_parameters = parameters.copy()
            new_parameters['imputation_method'] = None
            new_parameters['imputation_when'] = None
            new_parameters['name'] = combination_name_base(new_parameters)
            #print('Appending', new_parameters['name'], '\n')
            ##print(parameters)
            #print('BEFORE', grid[pos], 'BEFORE')
    ...
    new_parameters = {'model_regressor': 'tree',
                      ...
                      'custom_sample': 'mean',
                      'sample_removal': None,
                      'name': '00_treeEX_COMPARISON_con/con',
                      'pos': (False, False)}
    ...
    new_parameters = {'model_regressor': 'tree',
                      ...
                      'custom_sample': 'mean',
                      'sample_removal': None,
                      'name': '00_treeEX_COMPARISON_con/bin',
                      'pos': (False, False)}
    ...
    new_parameters = {'model_regressor': 'tree',
                      ...
                      'custom_sample': 'mean',
                      'sample_removal': None,
                      'name': '00_treeSN_COMPARISON_con/bin',
                      'pos': (False, False)}
    ...
    new_parameters = {'model_regressor': 'tree',
                      ...
                      'custom_sample': 'mean',
                      'sample_removal': None,
                      'name': '00_treeSN_COMPARISON_con/con',
                      'pos': (False, False)}
    ...
    imp_method = [None, 'mean', 'knn', 'mice']#, 'mice']
    #imp_method = ['mice']
    imp_when = ['before', 'after']
    for method in imp_method:
        if method is not None:
...
def main(dfname='iris', cross_validation=True): 
    ...
    if dfname == 'iris':
        configurations = select_mv_strategy(X_train, y_train, double=False, standard = 0.25)
    else:
        configurations = select_mv_strategy(X_train, y_train, double=False, standard = 0.25)
        ...
        for pl in MVplacements:
            ...
            #####FOR EACH PARAM COMBINATION:
                ...
                if pl != (False, False):
                    ...
                    train_path = dir_path+dfname+'_train.csv'
                    if os.path.exists(train_path):
                        print("Path already existing. No need to save train data csv for this dataset.")
                    else:
                        pd.DataFrame(train_dataset).to_csv(train_path, index=False, header=col_names)
                    explainer = limeMV.LimeTabularExplainerMV(train_dataset, #select correct train.
                                        ...
                                        to_round=to_encode) #model_regressor=None/'tree'
                    ...
                    '''for n, row in enumerate(test_dataset):
                        ...
                        print('Seconds passed:', round(time.time()-start, 4), ' \n')'''
                    print(test_dataset)
                    test_path = dir_path+dfname+'_test.csv'
                    if os.path.exists(test_path):
                        print("Path already existing. No need to save test data csv for this dataset.")
                    else:
                        pd.DataFrame(test_dataset).to_csv(test_path, index=False, header=col_names)
                    ...
                    if 'BASELINE' in comb['name']:
                        ...
                        if comb['imputation_when'] == 'before':
                            ...
                            elif comb['imputation_method'] == 'mice':
