...
class BaseDiscretizer():
    ...
    def __init__(self, data, categorical_features, feature_names, labels=None, random_state=None,
        ...
        for feature, qts in zip(self.to_discretize, bins):
            ...
            boundaries = np.nanmin(data[:, feature]), np.nanmax(data[:, feature]) #CHANGED FROM np.min, np.max
            ...
            self.names[feature].append('%s = NaN' % (name)) #NEW!!!!
            ...
            for x in range(n_bins + 1):
                ...
                mean = 0 if len(selection) == 0 else np.nanmean(selection) #CHANGED FROM np.mean
                if np.isnan(mean):
                    mean = max(bins[feature])
                self.means[feature].append(mean)
                std = 0 if len(selection) == 0 else np.nanstd(selection) #CHANGED FROM np.std
                if np.isnan(std):
                    std = 0
                std += 0.00000000001
    ...
    def get_undiscretize_values(self, feature, values):
        ...
        ret = minz
        '''
        print('minz', minz[min_max_unequal])
        print('maxz', maxz[min_max_unequal])
        print('means', means[min_max_unequal])
        print('stds', stds[min_max_unequal])
        print('randomstate', self.random_state)
        '''
        '''for mean_elements in means:
            pass'''
    ...
    def undiscretize(self, data):
        ret = data.copy()
        for feature in self.means: #medie delle features???
            if len(data.shape) == 1:
    ...
    def selective_undiscretize(self, data, idx):
        ret = data.copy()
        for feature in self.means: #medie delle features???
            if feature in idx:
                if len(data.shape) == 1:
...
class QuartileDiscretizer(BaseDiscretizer):
    ...
    def bins(self, data, labels):
        bins = []
        for feature in self.to_discretize:
            qts = np.array(np.nanpercentile(data[:, feature], [25, 50, 75])) #CHANGED from np.percentile
            bins.append(qts)
        return bins

class QuartileDiscretizerMV(BaseDiscretizer):
    def __init__(self, data, categorical_features, feature_names, labels=None, random_state=None):
    ...
    def bins(self, data, labels):
        bins = []
        for feature in self.to_discretize:
            values = data[:, feature][np.logical_not(np.isnan(data[:, feature]))]
            qts = np.array(np.nanpercentile(values, [25, 50, 75])) 
            bins.append(qts)
        bins.append(np.nan)
        return bins
...
class DecileDiscretizer(BaseDiscretizer):
    ...
    def bins(self, data, labels):
        bins = []
        for feature in self.to_discretize:
            qts = np.array(np.nanpercentile(data[:, feature],
                                         [10, 20, 30, 40, 50, 60, 70, 80, 90])) #CHANGED from np.percentile
            bins.append(qts)
        return bins
